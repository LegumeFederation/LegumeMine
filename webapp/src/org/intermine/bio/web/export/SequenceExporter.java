...
import org.intermine.model.bio.ConsensusRegion;
...
public class SequenceExporter implements Exporter {
        ...
        try {
            while (resultIt.hasNext()) {
                ...
                if (object instanceof SequenceFeature) {
                    if (extension > 0) {
                        bioSequence = createSequenceFeatureWithExtension(header, object, row, unionPathCollection, newPathCollection);
                    } else {
                        bioSequence = createSequenceFeature(header, object, row, unionPathCollection, newPathCollection);
                    }
                } else if (object instanceof Protein || object instanceof ConsensusRegion) {
    ...
    private BioSequence createProtein(StringBuffer header, Object object, List<ResultElement> row, Collection<Path> unionPathCollection, Collection<Path> newPathCollection)
    ...
    private BioSequence createSequenceFeature(StringBuffer header, Object object, List<ResultElement> row, Collection<Path> unionPathCollection, Collection<Path> newPathCollection)
        throws IllegalSymbolException {
        BioSequence bioSequence;
    ...
    private Sequence createSequenceFeatureWithExtension(StringBuffer header, Object object, List<ResultElement> row, Collection<Path> unionPathCollection, Collection<Path> newPathCollection)
        throws IllegalSymbolException {
